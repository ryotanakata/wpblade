...
- **出荷（人間用・レビュー→意味単位コミット→PR本文生成まで。push/PR作成はしない）**: `.claude/skills/wpblade-ship/SKILL.md`
- **PRテンプレート**: `.github/pull_request_template.md`。`## 受け入れ条件（AC）` の見出しは表記を変えない（AC の箇条書きを差分と照合する仕組みが前提のため）。`wpblade-ship` が人間向けに生成する本文にはAC欄が無いが、ルーチンが自動作成するPRは必ずこのテンプレートのAC欄を埋める
